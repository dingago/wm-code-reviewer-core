		...
		if (node.isEnabled()) {
			...
			if (lastPathItem.getType() == NSField.FIELD_STRING
				...
				for (int i = pathItems.length - 1; i >= 0; i--) {
					...
					/**
					 * Only do the checking when target field is under a
					 * referenced document.
					 */
					if (pathItem.getType() == NSField.FIELD_RECORDREF) {
						...
						if (nsRecord != null) {
							while (!fieldNames.isEmpty()) {
								...
								if (nsField == null) {
									/**
									 * Not found target field which is
									 * unexpected, but this is what does this
									 * rule care, so let other rules to
									 * complain.
									 */
									return null;
								} else {
									if (!fieldNames.isEmpty()) {
										if (nsField.getType() == NSField.FIELD_RECORD) {
											/**
											 * Not reaching the target field
											 * yet, keep looking for.
											 */
										...
										if (nsField.getType() == NSField.FIELD_STRING
											...
											/**
											 * Not found target field because
											 * the found field is not scalar
											 * string.
											 */
											return null;
										}
						...
						/**
						 * Not found target field in the closest referenced
						 * document type. No need to keep looking for.
						 */
						return null;
					} else {
						/**
